...
public class Main {
	...
	static void chooseFile(Parser parser, Scanner scanner) {
		...
		while(!isValid) {
			...
			try {
				parser.fileManipulation.input.setFile(fileName);
				isValid = true;
	...
	static void chooseOutputPath(Parser parser, Scanner scanner) {
		...
		while(!isValid) {
			...
			try {
				parser.fileManipulation.output.setPath(output);
				isValid = true;
	...
	static void openOutputFile(Parser parser, Scanner scanner) {
		...
		if(escolha.equals("s")) {
			try {
				Runtime.getRuntime().exec(new String[] {"xdg-open", parser.fileManipulation.output.getPath().toString()});
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
